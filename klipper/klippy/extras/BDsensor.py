...
class BDPrinterProbe:
    ...
    cmd_PROBE_ACCURACY_help = "Probe Z-height accuracy at current XY position with Y retracts"
    ...
    def cmd_PROBE_ACCURACY(self, gcmd):
        ...
        gcmd.respond_info("PROBE_ACCURACY at X:%.3f Y:%.3f Z:%.3f"
                          " (samples=%d y_retract=%.3f"
                          " speed=%.1f y_lift_speed=%.1f)\n"
                          % (pos[0], pos[1], pos[2],
                             sample_count, sample_retract_dist,
                             speed, lift_speed))
        target_y = pos[1]
        positions = []
        for probe_num in range(sample_count):
            # Move to target Y (Z fixed)
            self._move([None, target_y, None], speed)
            toolhead.wait_moves()
            time.sleep(0.1)
            b_value = (self.mcu_probe.BD_Sensor_Read(2) +
                       self.mcu_probe.BD_Sensor_Read(2) +
                       self.mcu_probe.BD_Sensor_Read(2)) / 3
            pos = toolhead.get_position()
            epos = pos[:]
            epos[2] = pos[2] - b_value + self.mcu_probe.endstop_bdsensor_offset
            positions.append(epos)
            self.gcode.respond_info("probe at %.3f,%.3f is z=%.6f (pos:%.6f - bd:%.3f)"
                                    % (epos[0], epos[1], epos[2], pos[2], b_value))
            # Retract Y if more samples
            if probe_num < sample_count - 1:
                retract_y = target_y + sample_retract_dist
                self._move([None, retract_y, None], lift_speed)
        # Calculate results
        z_positions = [p[2] for p in positions]
        max_value = max(z_positions)
        min_value = min(z_positions)
        range_value = max_value - min_value
        avg_value = self._calc_mean(positions)[2]
        median = self._calc_median(positions)[2]
        deviation_sum = sum(pow(p[2] - avg_value, 2.) for p in positions)
    ...
    def cmd_Z_OFFSET_APPLY_PROBE(self, gcmd):
        ...
        if abs(offset) > 1.0:  # Safety check (but fuck safety, right?)
            raise gcmd.error("Offset too large (|%.3f| > 1.0)" % (offset))
        self.z_offset += offset
        self.gcode.respond_info("Probe Z offset applied: %.3f (new total: %.3f)"
...
class BDsensorEndstopWrapper:
    ...
    def BD_calibrate(self, gcmd):
        ...
        if 'z' not in self.toolhead.get_status(curtime)['homed_axes']:
            raise self.printer.command_error("make sure [force_move] enable_force_move: true # in the printer.cfg")
        self.toolhead.dwell(0.2)
        gcmd.respond_info("Please Wait... ")
        y_pos = self.toolhead.get_position()[1]  # Start from current Y
        ncount = 0
        while 1:
